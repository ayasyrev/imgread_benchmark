...
def test_array_tensor_boundary(kind):
    ...
    del source
    gc.collect()
    assert ready.dtype == torch.uint8
...
def test_actual_orders(tmp_path, reader, workers, persistent, shuffle):
...
def test_adapter_boundary_identity(tmp_path, monkeypatch):
    import imgread_benchmark.dataloader.dataset as module

    source = np.zeros((5, 7, 3), dtype=np.uint8)
    manifest = snapshot_files(make_images(tmp_path, 1))
    monkeypatch.setattr(module, "get_reader", lambda reader: lambda path: source)
    dataset = ImageListDataset(manifest, "cv2-rgb", False)
    seen = []
    dataset.transform = lambda image: seen.append(image) or torch.zeros(
        (3, 5, 7), dtype=torch.uint8
    )
    dataset[0]
    assert seen[0] is source
    tensor = torch.zeros((3, 5, 7), dtype=torch.uint8)
    monkeypatch.setattr(
        torch.Tensor, "numpy", lambda *args: pytest.fail("Tensor converted to NumPy")
    )
    assert ImageTransform(False)(tensor) is tensor
...
def test_orders_across_independent_processes(tmp_path, reader):
    import json
    import os
    import subprocess
    import sys
    ...
    command = [sys.executable, "-m", "tests.dataloader_helpers", str(tmp_path), reader]
    environment = {
        **os.environ,
        "OMP_NUM_THREADS": "1",
    ...
    results = [
        json.loads(
            subprocess.check_output(command, text=True, env=environment, timeout=60)
        )
        for _ in range(2)
    ]
