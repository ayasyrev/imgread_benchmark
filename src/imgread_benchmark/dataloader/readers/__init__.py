"""Explicit file/bytes paths; capability probes run in disposable subprocesses."""
...
from ..models import READER_IDS, ReaderInfo, validate_storage
...
_DESCRIPTIONS = (
    ...
    "imgread RGB → ndarray (files or encoded bytes)",
    "Persistent imgread.Loader RGB → ndarray (files or encoded bytes)",
)


def get_reader(reader_id, storage="files"):
    ...
    paths = {
        ...
        "imgread-rgb": ("imgread", "read"),
        "imgread-loader-rgb": ("imgread", "persistent_reader"),
    }
    ...
    if storage == "memory":
        name += "_bytes"
    reader = getattr(importlib.import_module(f"{__name__}.{module}"), name)
    return reader() if reader_id == "imgread-loader-rgb" else reader


def check_runtime(reader_id, storage="files"):
    import torch
    import torchvision
    ...
    elif reader_id.startswith("imgread"):
        import imgread

        required = (
            ("Loader",)
            if reader_id == "imgread-loader-rgb"
            else ("load_numpy", "load_numpy_from_bytes")
        )
        for name in required:
            if not callable(getattr(imgread, name, None)):
                raise ValueError(
                    f"imgread missing required API {name}; install a build providing it"
                )
        version = imgread.__version__
...
def _probe():
    ...
    try:
        result = check_runtime(reader_id, sys.argv[2])
    except Exception as exc:
        result = ReaderInfo(
            reader_id,
...
def list_readers(storage="files"):
    return tuple(probe_reader(reader, storage) for reader in READER_IDS)
