...
def execute(manifest, config, execution_id, emit, wait_go):
    ...
    if config.warmup and config.storage == "files":
    ...
    try:
        dataset = (
            ImageListDataset(manifest, config.reader, config.geometry)
            if config.storage == "files"
            else ImageListDataset(
                manifest, config.reader, config.geometry, storage=config.storage
            )
        )
        loader = DataLoader(
        ...
        emit(
            ...
            preparation=dict(
                ...
                warmup=config.warmup and config.storage == "files",
                storage=config.storage,
                encoded_bytes=dataset.encoded_bytes
                if config.storage == "memory"
                else 0,
                memory_ownership="consumer bytes; copied to each spawned worker"
                if config.storage == "memory"
                else None,
            ),
        )
